...
def create_python_container(image_name):
    return str(subprocess.check_output(f"docker run -td {image_name} tail", shell=True))[2:-3]
...
def initialize():
    if not CONFIGURATION["DOCKER_CONTAINER_ID"]:
        CONFIGURATION["DOCKER_CONTAINER_ID"] = create_python_container("training-exercises_language-py")
